...
In addition, the three-statement semicolon separated for loop is gone in favor of a `for assignment to boolean` like statement. Removing the assignment makes `for to` act like a `do ... while` loop. In addition a `goto` statement is back. Perhaps array indicies will start at one now to prevent off by one errors?
